...
func (f *Frame) UnmarshalBinary(r ByteReader) error {
	...
	// Cap frame length to int32 because a golang int is either 32 or 64 bits
	// and it cannot allocate an array larger than math.MaxInt
	if frameLength > math.MaxInt32 {
		return fmt.Errorf("frameLength is too larger")
	}
